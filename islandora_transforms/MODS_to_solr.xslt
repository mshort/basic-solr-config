...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:foxml="info:fedora/fedora-system:def/foxml#" xmlns:mods="http://www.loc.gov/mods/v3" exclude-result-prefixes="mods">
    ...
    <xsl:template match="mods:mods">
        ...
        <!-- Series fields for story papers, which lack mods:relatedItem[@type='series'] -->        
        <xsl:if test="mods:physicalDescription/mods:form='Story papers'">
            <field>
                <xsl:attribute name="name">
                    <xsl:value-of select="concat($prefix, 'series_title_proper', $suffix)"/>
                </xsl:attribute>
                <xsl:if test="mods:titleInfo[not(@*)]/mods:nonSort">
                    <xsl:value-of select="mods:titleInfo[not(@*)]/mods:nonSort"/>
                    <xsl:text> </xsl:text>
                </xsl:if>
                <xsl:value-of select="mods:titleInfo[not(@*)]/mods:title"/>
            </field>
            <field>
                <xsl:attribute name="name">
                    <xsl:value-of select="concat($prefix, 'series_title_preferred', $suffix)"/>
            ...
            <field>
                ...
                <xsl:if test="mods:titleInfo[not(@*)]/mods:partName">
                    <xsl:text>. </xsl:text>
                    <xsl:value-of select="mods:titleInfo[not(@*)]/mods:partName"/>
                </xsl:if>
                <xsl:text> ; </xsl:text>
                <xsl:value-of select="mods:titleInfo[not(@*)]/mods:partNumber"/>
            </field>
            <field>
                <xsl:attribute name="name">
                    <xsl:value-of select="concat($prefix, 'series_statement_preferred', $suffix)"/>
            ...
            <field>
                ...
                <xsl:value-of select="substring-after(mods:titleInfo[not(@*)]/mods:partNumber,'no. ')"/>
            </field>
        </xsl:if>
        <!-- Host Name -->
